...
$windowsZip = "$artifactsDir\sourdough_windows_$version.zip"
...
$apkDest = "$artifactsDir\sourdough_android_$version.apk"
Copy-Item $apkSource $apkDest -Force
Write-Host "[Android] Artifact created: $apkDest" -ForegroundColor Green

# --- WEB & DEPLOY ---
Write-Host "`n[Web] Building Web (Base URL: /sourdough/)..." -ForegroundColor Cyan

# Build with base-href for GitHub Pages
flutter build web --release --base-href "/sourdough/"
if ($LASTEXITCODE -ne 0) { Write-Error "Web build failed"; exit 1 }

$webBuildDir = "build\web"
$webZip = "$artifactsDir\sourdough_web_$version.zip"
...
try {
    ...
    Write-Host "App URL: https://Will-Beninger.github.io/sourdough/" -ForegroundColor Cyan
}
catch {
